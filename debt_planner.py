...
def build_plan(
    ...
    # 4b. Keep the simulator timeline as the source of truth.
    # The expanded daily/weekly view is only for debugging or a separate detailed export.
    freq_map = {L.name: L.payment_frequency for L in enriched}
    detailed_timeline = _expand_timeline(
        sim_result.timeline,
        freq_map,
        {L.name: L.payment_amount for L in enriched},
        {
    ...
    # If you want, you can print or inspect detailed_timeline here,
    # but do NOT overwrite sim_result.timeline with it.
    # sim_result.timeline stays clean: one simulator row per loan per month.
